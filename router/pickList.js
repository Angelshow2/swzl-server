...
router.post('/item/picklist', (req, res) => {
  ...
  pool.getConnection((err, conn) => {
    conn.query(`${currentSql} itemdesc LIKE '%${searchText}%'`, (e, r) => {
      if(e) throw e
      if(r) {
        totalNum = r.length
        totalPage = Math.ceil(totalNum / pageSize)
        pool.getConnection((err, conn) => {
          conn.query(`${currentSql} itemdesc LIKE '%${searchText}%' ORDER BY publish_time DESC LIMIT ${start},${pageSize}`, (e, r) => {
            if(e) throw e
            if(r) {
